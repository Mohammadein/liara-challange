...
CASES: list[dict] = [
    ...
    {
        "group": "grounding",
        "name": "تغییر شبکه خصوصی — دو سرویس، دو راه‌حل متفاوت",
        "body": {"question": "چطوری شبکه‌ای که سرویس درش هست رو بعد از استقرار "
                             "اولیه تغییر بدم؟"},
        "expect": {
            # ادعای اصلی باید بیاید
            "answer_any": ["امکان پذیر نیست", "امکان‌پذیر نیست", "وجود ندارد",
                           "نمی‌توان", "قابل تغییر نیست"],
            "url_any": ["private-network"],
        },
        "note": "مستندات برای PaaS می‌گوید «مجدد استقرار» و برای دیتابیس "
                "«بازیابی فایل پشتیبان». ادغام این دو باعث می‌شود به کاربر "
                "بگوییم دیتابیسش را redeploy کند — یعنی از دست دادن دیتا.",
    },
    {
        "group": "grounding",
        "name": "تغییر شبکه خصوصی دیتابیس — راه‌حل درست بکاپ است",
        "body": {"question": "شبکه خصوصی دیتابیسم رو می‌خوام عوض کنم، چیکار کنم؟"},
        "expect": {
            "answer_any": ["پشتیبان", "بکاپ", "backup", "بازیابی"],
            "answer_none": ["مجدد استقرار انجام دهید"],
        },
    },
    ...
    # ---------------------------------------------------------- ابزارها
    # ⚠️ این‌ها فقط روی /api/chat معنی دارند (ابزارها آنجا فعال‌اند).
    #    api_v1/ask عمداً بدون ابزار و قطعی است. برای تست ابزارها:
